...
import { pickCityToRaid } from "../../helper/game-tile-event-attack-logic"
...
export default function PlayerSettingAttackCity() {
    ...
    gameState.gamePlayerInfo.map(v => {
        if(v.display_name != gameState.myPlayerInfo.display_name) {
            // set my city list
            // tempCityInfo = v.city
            const cityAmount = v.city.split(';').length
            // loop city
            ...
            for(let dci of dataCityInfo) {
                ...
                const attackCityData = {}
                v.city?.match(cityName) ? attackCityList.push(`${cityOwner},${cityAmount},${cityName},${cityPrice}`) : null
            }
        }
        else {
        ...
        absolute top-9 bg-darkblue-2 w-[calc(100%-.5rem)] h-[calc(100%-2.5rem)]`}>
            <div className="flex flex-col p-1">
                ...
                <div className="flex flex-col gap-2 h-36 lg:h-[17rem] overflow-y-scroll">
                    {attackCityList.map((v,i) => {
                        const [cityOwner, cityAmount, cityName, cityPrice] = v.split(',')
                        const translatedCityName = translateUI({lang: miscState.language, text: cityName as any}) 

                        return (
                            <form key={i} onSubmit={ev => pickCityToRaid(ev, setShowAttackConfirmation, v.split(','))} 
                ...
                {/* attack confirmation popup */}
                <form onSubmit={ev => ev.preventDefault()} className={`${showAttackConfirmation ? 'flex' : 'hidden'}
                flex-col justify-center absolute bg-darkblue-1 w-[calc(100%-.5rem)] h-[calc(100%-0.5rem)] p-1`}>
                    <div className="flex flex-col gap-4 grow justify-center">
                        ...
                        <div className="flex justify-around">
                            <button type="submit" className="text-green-400 border rounded-md p-1 w-14 lg:w-24 h-6 lg:h-10 hover:bg-darkblue-2"> quake </button>
                            <button type="submit" className="text-green-400 border rounded-md p-1 w-14 lg:w-24 h-6 lg:h-10 hover:bg-darkblue-2"> meteor </button>
                            <button type="submit" className="text-green-400 border rounded-md p-1 w-14 lg:w-24 h-6 lg:h-10 hover:bg-darkblue-2"> steal </button>
                        </div>
                    </div>
                    <div className="flex justify-center border-t-2">
                        <button type="button" className="p-1" onClick={() => setShowAttackConfirmation(false)}>
